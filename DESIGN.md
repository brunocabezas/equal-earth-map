...
### Navigation
Night bar, cream brand serif, thin-line globe mark, How Mercator stretches menu before the ice search well, then labeled Layers and About pills and a Language menu. On ≤1100px the tagline hides, that control shortens to Mercator stretch, and Language shortens to EN / ES / FR / PT / AR / ZH / RU. On ≤720px search becomes a labeled Search pill; the field opens as an ice overlay under the bar, and Mercator, Search, Layers, and Language menus center under the bar. The compact bar wraps onto a second row if the controls cannot fit. At 360px and below the serif name hides and Mercator shortens to the word Mercator so the bar stays one row; Search is icon-only; Language keeps its two-letter code. Skip to map focuses the globe; an ink plus and an inset coral ring mark keyboard focus. Escape leaves the map and hides them. Wall-map downloads live in About as ice pills.
...
Ice card under the How Mercator stretches pill with Every country / When I point / Hide comparison as a menu, and a short note under that menu (not inside it). On compact and hover-less pointers that control is When I tap: tap a country to zoom, open the place card, and keep Mercator’s hatch. At rest every inflated country’s hatch is at full strength so the ghosts read as area on the ocean; selecting a country keeps that hatch and greys the others. Not a bottom sheet; it stays a dropdown on compact, centered under the night bar.
...
### Compact search
On desktop the night bar has only the ice search field, not a separate Search button. On ≤720px that field hides and a Search pill (icon + label) sits between Mercator and Layers; it opens the same ice field as a layer above the map. At 360px and below Layers, About, and Search go icon-only; Mercator keeps the word Mercator. Escape and tapping the dimmed map close it.

### Zoom stack
42×42 (40×40 on small screens) paper buttons in a 12px cluster with hairline dividers.

### Center cluster
Horizontal ice pill of Africa / Americas / Pacific, inset padding, legend-lift shadow, ink fill on the active cell. Floats bottom-right on the map on viewports wider than 720px. On compact it hides there and the same three centers live in Layers. 44px tall on coarse pointers.
